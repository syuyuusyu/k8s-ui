...
export default class QuotaStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/quota');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/ResourceQuota`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/ResourceQuota/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
