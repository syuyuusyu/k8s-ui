...
export default class PvcStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/pvc');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/PersistentVolumeClaim`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/PersistentVolumeClaim/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
