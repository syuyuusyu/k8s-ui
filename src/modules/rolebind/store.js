...
export default class RbStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/rb');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/RoleBinding`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/RoleBinding/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
