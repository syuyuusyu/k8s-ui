...
export default class SecretStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/secret');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/Secret`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/Secret/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
