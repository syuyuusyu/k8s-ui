...
export default class RStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/r');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/Role`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/Role/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
