...
export default class RsStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/rs');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/ReplicaSet`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/ReplicaSet/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
