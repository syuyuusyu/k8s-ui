...
export default class HpaStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/hpa');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/HorizontalPodAutoscaler`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/HorizontalPodAutoscaler/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
            this.rootStore.history.push('/k8s/pha')
        } else {
            notification.error({
                message: json.msg,
                style: {
