...
export default class StsStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/sts');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/StatefulSet`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/StatefulSet/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
