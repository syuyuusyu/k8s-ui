...
export default class CjStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push(`/k8s/${kind}`);
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/CronJob`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/CronJob/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
            this.rootStore.history.push('/k8s/cj')
        } else {
            notification.error({
                message: json.msg,
                style: {
