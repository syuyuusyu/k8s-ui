...
import { get, del, put } from '../../config/util'
...
export default class DeployStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/deploy');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/Deployment`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/Deployment/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
