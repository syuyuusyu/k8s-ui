...
export default class PvStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/pv');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/PersistentVolume`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/PersistentVolume/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
