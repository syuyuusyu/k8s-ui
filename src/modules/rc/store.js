...
export default class RcStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/rc');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/ReplicationController`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/ReplicationController/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
