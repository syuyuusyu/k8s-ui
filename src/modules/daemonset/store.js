...
export default class DsStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/ds');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/DaemonSet`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/DaemonSet/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
