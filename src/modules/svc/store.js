...
export default class SvcStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/svc');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/Service`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/Service/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
