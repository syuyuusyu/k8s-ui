...
export default class LimitsStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/limits');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/LimitRange`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/LimitRange/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
