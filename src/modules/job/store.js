...
export default class JobStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/job');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/Job`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/Job/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
