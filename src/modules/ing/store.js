...
export default class IngStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/ing');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/Ingress`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/Ingress/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
