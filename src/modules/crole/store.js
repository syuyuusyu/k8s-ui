...
export default class CrStore {
    ...
    get currentElement() {
        ...
        if (!e) {
            this.rootStore.history.push('/k8s/cr');
    ...
    update = async () => {
        ...
        let json = await put(`${host}/kube/ClusterRole`, this.yamlJson);
    ...
    delete = async () => {
        let json = await del(`${host}/kube/namespace/${this.rootStore.columnStore.currentNamespace}/ClusterRole/${this.currentName}`);
        if (json.success) {
            notification.info({
                message: json.msg
            });
